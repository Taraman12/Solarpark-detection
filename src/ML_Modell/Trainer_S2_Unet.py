...
class TrainerS2Unet:
    ...
    def train_model(
        self,
        model_dir: Path,
        pretrained_model_path: Union[Path, None] = None,
        ...
        train_dataloader = DataLoader(
            train_ds, batch_size=self.batch_size, shuffle=self.shuffle, num_workers=0
        )

        test_dataloader = DataLoader(
            test_ds, batch_size=self.batch_size, shuffle=self.shuffle, num_workers=0
        )
    ...
    def write_to_csv(self, avg_jaccard_idx: float) -> None:
    ...
    def test(
        ...
        return loss.item(), jaccard_idx.item()  # type: ignore
